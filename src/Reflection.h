...
namespace Reflection
{
	...
	constexpr auto GetStaticMethod(const char* Name) -> ReturnType
	{
		...
		if (value)
		{
			...
			{
				std::cerr << exception.what() << " for " << Name << " static method in the class " << typeid(ObjectType_).name()
	...
	constexpr auto GetStaticMember(const char* Name) -> ReturnType
	{
		...
		if (value)
		{
			...
			{
				std::cerr << exception.what() << " for " << Name << " static member in the " << typeid(ObjectType_).name()
	...
	void InvokeMethod(ObjectType_& Object, const char* Name, Args&&... args)
	{
		...
		if (!Method)
		{
			throw std::exception((std::string("No method named ") + Name + " found in the object " + typeid(ObjectType_).name() + ".").c_str());
		}
